...
export const getUserByEmailOrMobile = async (email: string, mobile: string) => {
  await db.read();
  return db.data?.users.find(
    (user) => user.email === email || user.name === mobile
  );
};

// Update user (disallow mobile)
export const saveUserUpdate = async (
  id: string,
  ...
  if (user) {
    ...
    if (updates.otp !== undefined) user.otp = updates.otp;
